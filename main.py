...
class Main():
    def __init__(self, train_config, env_config, debug=False):
        ...
        train_orig = pd.read_csv(f'./data/{dataset}/train.csv', sep=',', index_col=0) #, nrows=250000)
        test_orig = pd.read_csv(f'./data/{dataset}/test.csv', sep=',', index_col=0, nrows=1000)

        print("Datasets loaded!!!")
        print("NAs  = ", train_orig.isna().sum())
        print(train_orig.columns)

        train_orig.iloc[:,:-1] = winsorize(train_orig.iloc[:,:-1].values, limits=(0.05, 0.05), axis=0)
        train_orig.iloc[:,:-1] = StandardScaler().fit_transform(train_orig.iloc[:,:-1])
